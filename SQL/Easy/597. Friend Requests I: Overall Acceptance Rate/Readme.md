...
with num as (
select
    count(distinct concat(requester_id, accepter_id)) as accept_rate
from RequestAccepted
),

deno as (
    select
        count(distinct concat(sender_id, send_to_id)) as request_cnt
    from FriendRequest
)

select
    coalesce(round(accept_rate / request_cnt, 2), 0) as accept_rate
from num
join deno
```